...
- 开始界面和游戏顶栏均可在中文/English 之间即时切换；规则、随机序列和存档完全共用；
- 13 张行动卡直接使用网页版运行时 `ACTIONS` 文案，而不是会被脚本覆盖的静态 HTML 占位文案；点击与 15 个动作快捷键均按原 action ID 对接；
- 39 个基础事件、全部情境/灾变事件与 21 个 SPEC 事件恢复网页版正文、出处和实际变化播报，中英文共享相同的触发与结算；
- A–J 结局恢复网页版正文、引文、结局页、种子复制、终局观测与跨局终局统计；
...
- 以 GitHub `v0.2` 提交为固定历史基准，逐项核对运行时按钮文案、92 段事件正文与 A–J 结局内容；
